<template>
  <div v-if="manifest">
    <PageHeader
      title="Security"
      :subtitle="`Findings from static security artifacts · ${filtered.length} of ${manifest.security.length} shown`"
    >
      <StatusChip
        :status="criticalOrHigh ? 'failed' : 'passed'"
        :label="`${criticalOrHigh} critical/high`"
      />
    </PageHeader>
    ...
    <div class="toolbar">
      ...
      <v-select
        v-model="severity"
        :items="['all', 'critical', 'high', 'medium', 'low', 'info', 'unknown']"
        label="Severity"
        density="compact"
        hide-details
      />
      ...
      <v-expansion-panel v-for="finding in filtered" :key="finding.id" class="portal-card">
        ...
        <v-expansion-panel-text>
          <dl class="detail-list">
            <dt>Rule ID</dt>
            <dd class="mono">{{ finding.ruleId ?? "n/a" }}</dd>
            <dt>Message</dt>
            <dd>{{ finding.message ?? "n/a" }}</dd>
            <dt>Description</dt>
            <dd>{{ finding.description ?? "n/a" }}</dd>
            <dt>Confidence</dt>
            <dd>{{ finding.confidence ?? "n/a" }}</dd>
            <dt>Precision</dt>
            <dd>{{ finding.precision ?? "n/a" }}</dd>
            <dt>Risk Code</dt>
            <dd class="mono">{{ finding.riskCode ?? "n/a" }}</dd>
            <dt>CWE / WASC</dt>
            <dd class="mono">{{ cweWasc(finding) }}</dd>
            <dt>Tags</dt>
            <dd>
              <v-chip
            ...
            <dt>Location</dt>
            <dd class="mono">{{ location(finding) }}</dd>
            <dt>Evidence</dt>
            <dd class="mono wrap-anywhere">{{ finding.evidence ?? "n/a" }}</dd>
            <dt>Remediation</dt>
            <dd>{{ finding.remediation ?? "n/a" }}</dd>
            <dt>Help URI</dt>
            <dd>
              <a v-if="finding.helpUri" :href="finding.helpUri" target="_blank" rel="noopener">{{
                finding.helpUri
              }}</a
            ...
            <dd>
              <a
                v-if="rawReportFor(finding)"
                :href="rawReportFor(finding)!.path"
                target="_blank"
                rel="noopener"
                >{{ rawReportFor(finding)!.name }}</a
              >
...
const tool = ref("all");
const criticalOrHigh = computed(
  () =>
    (props.manifest?.summary.security.critical ?? 0) + (props.manifest?.summary.security.high ?? 0)
);
const tools = computed(() => [
  "all",
  ...new Set((props.manifest?.security ?? []).map((finding) => finding.tool))
]);
const toolSummaries = computed(() =>
  tools.value
    ...
    .map((item) => ({
      tool: item,
      count: props.manifest?.security.filter((finding) => finding.tool === item).length ?? 0
    }))
);
const severityBreakdown = computed<
  Array<{ label: string; value: number; class: "" | "low" | "medium" }>
>(() =>
