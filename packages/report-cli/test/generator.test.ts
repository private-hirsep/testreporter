...
describe("report generator", () => {
  it("uses completed chronological approved manual evidence and validates all evidence references", async () => {
    ...
    await mkdir(path.join(input, "cases"), { recursive: true }); await mkdir(path.join(input, "results"), { recursive: true }); await mkdir(path.join(input, "evidence"), { recursive: true }); await mkdir(path.join(input, "requirements"), { recursive: true });
    ...
    const config=loadConfigFromObject({project:{name:"APP"},artifacts:{manual:{cases:"cases/*.yml",results:"results/*.json",evidence:"evidence/*"},requirements:{expectedKeys:"requirements/expected.csv"}},requirements:{keyPattern:"REQ-[0-9]+"},qualityGates:{manual:{failOnFailed:true,requireCompleted:true}}});
    const report=await buildReport({config,configPath:"quality-report.yml",inputPath:input,outputPath:output});
    expect(report.manualExecutions.map((item)=>item.executionId)).toEqual(["older","newer","latest"]);
    expect(report.summary.manual).toMatchObject({cases:2,passed:1,failed:1,missingEvidence:1});
    expect(report.requirements.manualCasesByRequirement["REQ-1"]).toEqual(["APP-MT-1","APP-MT-3"]);
    expect(report.requirements.latestManualResultByRequirement["REQ-1"]).toBe("passed");
    expect(report.requirements.covered).toContain("REQ-1");
