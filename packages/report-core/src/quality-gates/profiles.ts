...
export const BuiltInQualityProfileSchema = z.enum(["off", "relaxed", "standard", "strict", "release"]);
...
export type QualityGateOverrides = {
  tests?: { allowFailed?: number | undefined; allowBroken?: number | undefined } | undefined;
  coverage?:
    | { totalMinimum?: number | undefined; backendMinimum?: number | undefined; frontendMinimum?: number | undefined }
    | undefined;
  requirements?:
    | { minimum?: number | undefined; failOnMissing?: boolean | undefined; failOnExtra?: boolean | undefined }
    | undefined;
  security?:
    | { maxCritical?: number | undefined; maxHigh?: number | undefined; maxMedium?: number | undefined }
    | undefined;
  warnings?: { maxWarnings?: number | undefined } | undefined;
};
...
export const BUILT_IN_QUALITY_PROFILES: Record<BuiltInQualityProfile, QualityGateSettings & { enabled: boolean }> = {
  off: {
    enabled: false,
...
function mergeQualityGates(base: QualityGateSettings, override?: QualityGateOverrides): QualityGateSettings {
...
export function resolveQualityProfile(
  ...
  const resolve = (name: string): { enabled: boolean; qualityGates: QualityGateSettings } => {
    if (seen.has(name)) throw new Error(`Quality gate profile "${name}" has a circular extends chain.`);
    seen.add(name);

    const builtIn = BUILT_IN_QUALITY_PROFILES[name as BuiltInQualityProfile];
    if (builtIn) {
      const { enabled, ...qualityGates } = builtIn;
      return { enabled, qualityGates: clone(qualityGates) };
    }
