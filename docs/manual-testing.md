...
Open **Manual Testing** in a generated report, choose a case, and work through the runner. Progress is saved in browser `localStorage`, keyed by project, release, execution ID, and tested build. It survives reloads, can be discarded, and contains references—not embedded attachments. A local draft is convenience state, not audit evidence.
